...
This script loads in the results from running (stochastic) batch simulations in VCell
...
dir_batchres = cwd * "/raw_batch_results/batchResults_stoch_-sor"  # directory containing batch results from VCell
...
save(cwd0 * "/plots/VCell_stoch_res_-sfnb_PLSR_R2+Q2.png", fig_r2q2, px_per_unit = 3)
save(cwd0 * "/plots/VCell_stoch_res_-sfnb_PLSR_R2+Q2.pdf", fig_r2q2, pt_per_unit = 1)
    ...
    (mapping([0]) * visual(HLines, linestyle = :dash) +     # horizontal line at y = 0
        data(_) * (
            ...
            mapping(:variable => sorter(unique(_.variable)), :μ) *
                visual(Scatter, color = aog.wongcolors()[1])
        )
    ) |>
    draw(_,
        axis = (ylabel = "PLSR coefficient",
                title = "LHS-PLSR SA, membrane RAF1\n(198 stochastic sims, -sorafenib)",
                xticklabelrotation = pi/2, height = 200, width = 600))
display(fig_coefs_boot)
save(cwd0 * "/plots/VCell_stoch_res_-sfnb_PLSR_bootstrapped_coefficients.png", fig_coefs_boot, px_per_unit = 3)
save(cwd0 * "/plots/VCell_stoch_res_-sfnb_PLSR_bootstrapped_coefficients.pdf", fig_coefs_boot, pt_per_unit = 1)
...
save(cwd0 * "/plots/VCell_stoch_res_-sfnb_PLSR_bootstrapped_VIP_scores.png", fig_vip_boot, px_per_unit = 3)
save(cwd0 * "/plots/VCell_stoch_res_-sfnb_PLSR_bootstrapped_VIP_scores.pdf", fig_vip_boot, pt_per_unit = 1)