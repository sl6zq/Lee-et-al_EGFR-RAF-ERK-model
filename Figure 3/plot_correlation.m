function plot_correlation(number, sampling, data_matlab, data_sloppy, names)
    ...
    for i=1:size(data_matlab,2)
        ...
        plot(data_matlab(:,i),data_sloppy(:,i),'o','MarkerFaceColor','k', 'MarkerEdgeColor','k','MarkerSize',5);
        hold on
        l = lsline ;
        ...
        matlab_strong = find(abs(data_matlab(:,i)) > 0.4);
        sloppy_strong = find(abs(data_sloppy(:,i)) > 0.4);
        ...
        labelpoints(data_matlab(both_strong,i),data_sloppy(both_strong,i), names(both_strong), 'SE', 0.2, 1, 'FontSize', 8); %south east
        ...
        s1.XLim = [0 1];
        s1.YLim = [0 1];
        text(s1.XLim(:,2)-0.6,s1.YLim(:,2)-0.1, str1, 'FontSize',8);
        text(s1.XLim(:,2)-0.6,s1.YLim(:,2)-0.2, str2, 'FontSize',8);
