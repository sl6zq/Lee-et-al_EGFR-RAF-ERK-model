...
% Function to generate independent variable matrix (X) that consists of
% number of parameter sets ("number") as rows, number of parameters
% (length of "wanted_params") as columns and solve ODEs with each parameter
% set (row)

% set seed for random and LHS sampling
rng(1);
% Matrix to store parameter sets (X)
...
output = cell(9,1);
% Matrix to maximum model outputs values (Y)
...
% Matrix to store time-avg model outputs values (Y)
...
% Matrix to store time-avg model outputs (Y)
...
% Matrix to store time-integrated model outputs (Y)
...
% Cells to store model outputs (Y)
my_y_max_total           = cell(6,1); 
