function [output1, T, allValues, param, param_vary] = fullEGFR9_onemodel_fit_40_v2(argparam_vary, argTimeSpan, dataname, wanted_param, argYinit, argParam)
...
param = [
	...
	1.61E-02; %0.1;		% param(127) is 'kdEr'
	3.141592653589793;		% param(128) is 'mlabfix_PI_'
	1.09E-01; %9.2E-4;		% param(129) is 'kdEf'
	3.11E+01; %0.6;		% param(130) is 'kdpR1'
	...
	2.18E+00; %460.0;		% param(143) is 'kG2r'
	...
	4.66E-05; %3.8E-4;		% param(147) is 'kG2f'
	...
	1.60E+00; %0.37;		% param(159) is 'knfpiBr'
	0.0;		% param(160) is 'Kr_Ras_Raf1_nfp'
	0.0;		% param(161) is 'Ras_iRaf1_init_molecules_um_2'
	7.80E+02; %0.046229858;		% param(162) is 'kfpR1r'
	3.80E-02; %0.8;		% param(163) is 'kEr'
];
%}
%{
if nargin >= 3
...
if nargin >= 1
    param_vary = argparam_vary;
    for m=1:length(wanted_param)
        param(wanted_param(m)) = param_vary(m);
    end
end
...
if strcmp(dataname, 'min_sor_rastimedata') == 1
	param(146) = 0; %sorafenib

    
    [T,Y] = ode23tb(@f2,timeSpan,yinit,odeset,param,yinit);
    % get the solution
    all = zeros(length(T), numVars);
    for i = 1:length(T)
        all(i,:) = getRow2(T(i), Y(i,:), yinit, param);
    end
    allValues = all;
    output1 = allValues(:,184);
end
if strcmp(dataname, 'min_sor_raftimedata') == 1
	param(146) = 0; %sorafenib

    %[T,Y] = ode15s(@f2,timeSpan,yinit,odeset,param,yinit);
    [T,Y] = ode23tb(@f2,timeSpan,yinit,odeset('RelTol',1e-6,'AbsTol',1e-6),param,yinit);
    all = zeros(length(T), numVars);
    ...
    allValues = all;
    output1 = allValues(:,157);
end

if strcmp(dataname, 'min_sor_perktimedata') == 1
    param(146) = 0; %sorafenib
    ...
    allValues = all;
    output1 = allValues(:,45);
end

if strcmp(dataname, 'min_sor_pmektimedata') == 1
    param(146) = 0; %sorafenib
    [T,Y] = ode23tb(@f2,timeSpan,yinit,odeset,param,yinit);
    % get the solution
    all = zeros(length(T), numVars);
    ...
    allValues = all;
    output1 = allValues(:,21);
end

if strcmp(dataname, 'plus_sor_rastimedata') == 1
    param(146) = 10;
    [T,Y] = ode23tb(@f2,timeSpan,yinit,odeset,param,yinit);
    % get the solution
    all = zeros(length(T), numVars);
...
if strcmp(dataname, 'plus_sor_raftimedata') == 1
    param(146) = 10;
    [T,Y] = ode23tb(@f2,timeSpan,yinit,odeset,param,yinit);
    % get the solution
    all = zeros(length(T), numVars);
...
% -------------------------------------------------------
% get row data
function rowValue = getRow2(t,y,y0,p)
	% State Variables
	Ras_iBRaf = y(1);
