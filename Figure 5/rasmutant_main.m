...
%ylabel({'|Time-integrated conc.|'; 'in mutatnt vs. WT'},'FontWeight','bold');
ylabel(str);
set(gca, 'xTick', 1:length(delta_group_rafinc_pway_nonzero_idx),'xTickLabel',pway_order_group(delta_group_rafinc_pway_nonzero_idx,2),'XTickLabelRotation',45);
...
h2 = figure;
linewidth_h2 = 3;
set(h2,'Position',[100 100 380 300]);
plot(T,allValues_wt(:,45), 'k', 'LineWidth', linewidth_h2); %WT
xlabel('Time with EGF (min)','FontSize',7);
ylabel('pERK (molec/cell)','FontSize',7);
ylim([0 4*10^5]);ax = gca; ax.FontSize = 7; hold on
plot(T,allValues_mutant(:,45),'LineWidth', linewidth_h2, 'Color', 'r'); %KRAS mutant 
plot(T,allValues_wt_raf_highexp(:,45),":", 'LineWidth', linewidth_h2, 'Color', 'k')%'Color',[0.4660 0.6740 0.1880]); %WT w/ RAF increase
plot(T,allValues_raf_highexp(:,45),":",'LineWidth', linewidth_h2, 'Color', 'r');%'Color', [0.4940 0.1840 0.5560]); %KRAS mutant w/ RAF increase
inBetween2 = [allValues_raf_highexp(:,45)', fliplr(allValues_mutant(:,45)')]; %KRAS G12V with 10-fold increase in [RAF1], KRAS G12V 
%inBetween2 = [allValues_raf_highexp(:,45)', fliplr(allValues_wt_raf_highexp(:,45)')]; %KRAS G12V with 10-fold increase in [RAF1], wild-type RAS with 10-fold increase in [RAF1]
x2 = [T', fliplr(T')];
s2 = fill(x2, inBetween2, 'r');
alpha(s2,.5);
inBetween = [allValues_wt_raf_highexp(:,45)', fliplr(allValues_wt(:,45)')]; %WT RAS with 10-fold increase in [RAF1], WT RAS with 10-fold increase in [RAF1]
s1 = fill(x2, inBetween, 'g'); alpha(s1,0.5);
legend('WT RAS', 'KRAS G12V', 'WT RAS, RAF1 increase', 'KRAS G12V, RAF1 increase','Location','northoutside','FontSize',7);
hold off
...
fold_diff3_WT      = trapz(T,allValues_wt_raf_highexp(:,45))-trapz(T,allValues_wt(:,45));
fold_diff4_mutant  = trapz(T,allValues_raf_highexp(:,45))-trapz(T,allValues_mutant(:,45));
RAF1_inc_effect    = fold_diff4_mutant/fold_diff3_WT;
...
hold on
plot(T,allValues_wt_RAS_nofdbk(:,45),'LineWidth', 1.5,'Color',[0/255 191/255 196/255]); %WT RAS, no fdback
plot(T,allValues_wt_raf_highexp_nofdbk(:,45),'--','LineWidth', 1.5,'Color',[0/255 191/255 196/255]); %WT w/ RAF increase
plot(T,allValues_mutant(:,45),'LineWidth', 1.5, 'Color', [248/255 118/255 109/255]); %KRASG12V
plot(T,allValues_KRASG12V_nofdbk(:,45),'LineWidth', 1.5,'Color',[0/255 191/255 196/255]); %KRASG12V, no fdback
plot(T,allValues_raf_highexp_nofdbk(:,45), '--', 'LineWidth', 1.5,'Color', [248/255 118/255 109/255]); %KRASG12V, 10-fold increase in [RAF1], no fdback
legend('WT RAS', 'WT RAS, no fdback', 'WT RAS, 10-fold increase in [RAF1], no fdback', 'KRASG12V', 'KRASG12V, no fdback', 'KRASG12V, 10-fold increase in [RAF1], no fdback','Location','northoutside','FontSize',8);
xlabel('Time (min)','FontSize',12, 'FontWeight','bold');
ylabel('pERK (molec/cell)','FontSize',12, 'FontWeight','bold');
ylim([0 4*10^5]);hold off
...
plot(T,allValues_wt_RAS_nofdbk(:,45),'LineWidth', 1.5,'Color',[0/255 191/255 196/255]); %WT RAS, no fdback
hold on
plot(T,allValues_wt_raf_highexp_nofdbk(:,45),'--','LineWidth', 1.5,'Color',[0/255 191/255 196/255]); %WT RAS, no fdback, RAF increase
plot(T,allValues_KRASG12V_nofdbk(:,45),'LineWidth', 1.5,'Color',[248/255 118/255 109/255]); %KRASG12V, no fdback
plot(T,allValues_raf_highexp_nofdbk(:,45), '--', 'LineWidth', 1.5,'Color', [248/255 118/255 109/255]); %KRASG12V, no fdback, 10-fold increase in [RAF1]
inBetween3 = [allValues_wt_RAS_nofdbk(:,45)', fliplr(allValues_KRASG12V_nofdbk(:,45)')]; %KRAS G12V with 10-fold increase in [RAF1], KRAS G12V 
x2 = [T', fliplr(T')];
s2 = fill(x2, inBetween3, 'r');
legend('WT RAS, no fdback', 'WT RAS, no fdback, 10-fold increase in [RAF1]', 'KRASG12V, no fdback', 'KRASG12V, no fdback, 10-fold increase in [RAF1]','Location','northoutside','FontSize',8);
xlabel('Time (min)','FontSize',12, 'FontWeight','bold');
ylabel('pERK (molec/cell)','FontSize',12, 'FontWeight','bold');
ylim([0 4*10^5]);
hold off
...
% add a vertical line for reduced difference between RAF1 baseline vs.
% 10-fold RAF1 abundance
h1 = figure;
set(h1,'Position',[100 100 330 300]);
plot(T,allValues_wt_raf_highexp(:,45),'--','LineWidth', 1.5,'Color',[0/255 191/255 196/255]); %WT
hold on
plot(T,allValues_wt_raf_highexp_nofdbk(:,45),'LineWidth', 1.5,'Color',[0/255 191/255 196/255]); %WT w/ RAF increase
plot(T,allValues_raf_highexp(:,45),'--','LineWidth', 1.5, 'Color', [248/255 118/255 109/255]); %KRAS mutant with RAF inc.
plot(T,allValues_raf_highexp_nofdbk(:,45), '--', 'LineWidth', 1.5,'Color', [248/255 118/255 109/255]); %KRAS mutant w/ RAF increase
xlabel('Time with EGF (min)','FontSize',7);
ylabel('pERK (molec/cell)','FontSize',7);
ylim([0 4*10^5]);ax = gca;ax.FontSize = 7; 
WT_mut_diff = abs(allValues_wt_raf_highexp(:,45)/max(allValues_wt_raf_highexp(:,45)) - allValues_raf_highexp(:,45)/max(allValues_raf_highexp(:,45)))*100;
%WT_mut_diff = abs(allValues_wt_raf_highexp(:,45) - allValues_raf_highexp(:,45));
dydy = diff(WT_mut_diff)./diff(T);
[row, col]  = find(WT_mut_diff>1);
Rmin = T(min(row)+1);
yint = allValues_raf_highexp(:,45);
ix = find(diff(sign(T-Rmin)));                                  % Find Approximate Index
idxrng = max(1,ix-1) : min(numel(T),ix+1);                      % Define Index Range
xRmin = interp1(T(idxrng),yint(idxrng), Rmin);                 % Y-Coordinate Of Intercept
yRmin = interp1(yint(idxrng),T(idxrng), xRmin);                  % X-Coordinate Of Intercept
plot(yRmin, xRmin, 'ok')    
plot([0 yRmin], [1 1]*xRmin, '--k'); 
plot([1 1]*yRmin, [0 1]*xRmin , '--k');                                     % Marker
legend('WT RAS, RAF1 increase', 'WT RAS, RAF1 increase, no fdback', 'KRAS G12V, RAF1 increase', 'KRAS G12V, RAF1 increase, no fdback','% \Delta (WT and KRAS G12V with RAF1 increase) > 1','Location','northoutside','FontSize',7);
hold off
...
new_init_param = [12000.0*0.01 12000.0*0.1 12000.0];
...
RAF1diff = cell(length(new_init_param),1);
...
for i=1:length(new_init_param)
    ...
    legend('100-fold decrease in [RAF1]','10-fold decrease in [RAF1]','[RAF1] = 12,000 molec/cell', 'FontSize', 7);
    title('EGFR dimer', 'FontSize', 7);
    xlabel('Time (min)', 'FontSize', 7);
    ylabel('EGFR dimer (molec/cell)', 'FontSize', 7);
    ylim([0 max(new_init_params_EGFRdimer_minsor,[],'all')+max(new_init_params_EGFRdimer_minsor,[],'all')*0.1]);
    hold on
    ...
    legend('100-fold decrease in [RAF1]','10-fold decrease in [RAF1]','[RAF1] = 12,000 molec/cell', 'FontSize', 7);
    title('Phosphorylated EGFR');
    xlabel('Time (min)','FontWeight','bold', 'FontSize', 7);
    ylabel('pEGFR (molec/cell)','FontWeight','bold', 'FontSize', 7);
    ylim([0 max(new_init_params_pEGFR_minsor,[],'all')+max(new_init_params_pEGFR_minsor,[],'all')*0.1])
    hold on
    ...
    title('GTP-bound RAS');
    xlabel('Time (min)','FontWeight','bold', 'FontSize', 7);
    ylabel('GTP-bound RAS (molec/cell)','FontWeight','bold', 'FontSize', 7);
    ylim([0 max(new_init_params_RasGTP_minsor,[],'all')+max(new_init_params_RasGTP_minsor,[],'all')*0.1]);
    hold on
    ...
    title('Membrane-bound RAF1');
    xlabel('Time (min)','FontWeight','bold', 'FontSize', 7);
    ylabel('Membrane-bound RAF1 (molec/cell)','FontWeight','bold', 'FontSize', 7);
    ylim([0 max(new_init_params_membRAF1_minsor,[],'all')+max(new_init_params_membRAF1_minsor,[],'all')*0.1]);
    hold on
    ...
    title('pRAF1');
    xlabel('Time (min)','FontWeight','bold', 'FontSize', 7);
    ylabel('pRAF1 (molec/cell)','FontWeight','bold', 'FontSize', 7);
    ylim([0 max(new_init_params_pRAF1_minsor,[],'all')+max(new_init_params_pRAF1_minsor,[],'all')*0.1]);
    hold on
    
    set(0, 'CurrentFigure', f6)
    set(gcf,'Position',[100 100 300 250]);
    plot(T, new_init_params_pMEK_minsor(:,i),'LineWidth',1.2, 'Color',C{i})
    legend('100-fold decrease in [RAF1]','10-fold decrease in [RAF1]','[RAF1] = 12,000 molec/cell','Location','southeast', 'FontSize', 7);
    title('pMEK');
    xlabel('Time (min)','FontWeight','bold', 'FontSize', 7);
    ylabel('pMEK (molec/cell)','FontWeight','bold', 'FontSize', 7);
    ylim([0 max(new_init_params_pMEK_minsor,[],'all')+max(new_init_params_pMEK_minsor,[],'all')*0.1]);
    hold on
...
%% Compare effects of limiting RAF1 expression vs. limiting EGF binding affinity
% pERK_ligand_100fold_effects   = trapz(T,new_params_pERK_minsor(:,3))/trapz(T,new_params_pERK_minsor(:,1));
% pERK_RAF1_100fold_effects = trapz(new_init_params_pERK_minsor(:,3))/trapz(T,new_init_params_pERK_minsor(:,1));
% pERK_ligand_10fold_effects    = trapz(T,new_params_pERK_minsor(:,3))/trapz(T,new_params_pERK_minsor(:,2));
% pERK_RAF1_10fold_effects  = trapz(new_init_params_pERK_minsor(:,3))/trapz(T,new_init_params_pERK_minsor(:,2));

pERK_ligand_100fold_effects = max(new_params_pERK_minsor(:,3))/max(new_params_pERK_minsor(:,1));
pERK_RAF1_100fold_effects   = max(new_init_params_pERK_minsor(:,3))/max(new_init_params_pERK_minsor(:,1));
pERK_ligand_10fold_effects  = max(new_params_pERK_minsor(:,3))/max(new_params_pERK_minsor(:,2));
pERK_RAF1_10fold_effects    = max(new_init_params_pERK_minsor(:,3))/max(new_init_params_pERK_minsor(:,2));

comb_effects = [pERK_ligand_10fold_effects pERK_RAF1_10fold_effects;...
    pERK_ligand_100fold_effects, pERK_RAF1_100fold_effects];
figure
set(gcf,'Position',[100 100 300 250]);
X_cat = categorical({'10-fold', '100-fold'});
X_cat = reordercats(X_cat,{'10-fold', '100-fold'});
bar(X_cat, comb_effects);
ylim([0 2]);
ylab_str = {['Fold decrease in max. pERK levels'], ['compared to baseline']};
ylabel(ylab_str, 'FontSize', 7); 
legend('Binding affinity of ligand to EGFR', 'RAF1 abundance', 'FontSize', 7);
